...
- OPENAI_REALTIME_MODEL: padrão gpt-realtime-2.1-mini, com raciocínio `low` para equilibrar inteligência, custo e baixa latência.
...
O backend recusa chaves com formato inválido pela rota protegida `/ready`, usando o código `OPENAI_KEY_INVALID`, antes de solicitar acesso ao microfone no aplicativo. Falhas da OpenAI por saldo zerado usam `OPENAI_CREDIT_EXHAUSTED`; limites temporários usam `OPENAI_RATE_LIMIT`.
